...
alpha_z_values = [alpha_z_base * fc for fc in fold_changes]
cmap = plt.get_cmap("tab10")
colors = cmap(np.linspace(0, 1, len(alpha_z_values)))

def idx(i, offset):
    return i * 9 + offset

def repressilator_system(t, y, Q, alpha_z_val):
    ...
    for i in range(N):
        ...
        dbi = alpha / ((1 + Ai[i] ** n) * (1 + Zi[i] ** n)) - bi[i]
...
for alpha_z_val, color in zip(alpha_z_values, colors):
    for Q in Q_values:
        np.random.seed(0)
        y0 = np.random.rand(N * 9 + N) * 0.1

        sol = solve_ivp(lambda t, y: repressilator_system(t, y, Q, alpha_z_val),
        ...
        combined_traces[(alpha_z_val, Q)] = (time, bi_matrix)
        average_traces[(alpha_z_val, Q)] = np.mean(bi_matrix, axis=1)
        ...
        for ax, q_val in zip(ax_full, Q_values):
            ...
            ax.set_title(f"Q={q_val} | alpha_z={alpha_z_val:.1f}")
            ax.set_ylabel("mRNA bi")
            ax.grid(True)
        ax_full[-1].set_xlabel("Time (s)")
        fig_full.tight_layout()
        fig_full.savefig(f"full_alpha_z_{alpha_z_val:.1f}.png")
        plt.close(fig_full)
        ...
        for ax, q_val in zip(ax_zoom, Q_values):
            ...
            ax.set_title(f"Zoom 0-50s | Q = {q_val} | alpha_z = {alpha_z_val:.1f}")
            ax.set_ylabel("mRNA bi")
            ax.grid(True)
        ax_zoom[-1].set_xlabel("Time (s)")
        fig_zoom.tight_layout()
        fig_zoom.savefig(f"zoom_alpha_z_{alpha_z_val:.1f}.png")
        plt.close(fig_zoom)

df_results = pd.DataFrame(results)
df_results.to_csv("alpha_z_scan_summary_metrics.csv", index=False)
print("Saved alpha_z_scan_summary_metrics.csv")
...
for Q in Q_values:
    ...
    for ax, q_val in zip(axes, Q_values):
        for (a_z, q_key), (t, mat) in combined_traces.items():
            if q_key == q_val:
                color = colors[alpha_z_values.index(a_z)]
                for i in range(N):
    ...
    fig_all.suptitle("All Cells: mRNA bi per alpha_z", fontsize=16)
    fig_all.tight_layout(rect=[0, 0, 1, 0.96])

    # Create the legend with consistent color coding
    legend_lines = [Line2D([0], [0], color=colors[i], lw=2) for i in range(len(alpha_z_values))]
    legend_labels = [f"αz={a:.1f}" for a in alpha_z_values]
...
# Combined plots - Average curve per alpha_z, separated by Q value
...
for i, Q in enumerate(Q_values):
    ...
    # Loop through each alpha_z value and plot the average curve
    for alpha_z_val, color in zip(alpha_z_values, colors):
        avg_trace = np.mean([average_traces[(alpha_z_val, Q)] for Q in Q_values], axis=0)
        ax_avg.plot(time, avg_trace, label=f"αz={alpha_z_val:.1f}", color=color)

    # Set titles, labels, and grid
    ax_avg.set_title(f"Average mRNA bi per alpha_z | Q = {Q}")
...
for Q in Q_values:
    ...
    # Set x-axis label for the last subplot
    axes[-1].set_xlabel("Time (s)")
    fig_all.suptitle("All Cells (Zoomed 0-50s): mRNA bi per alpha_z", fontsize=16)
    fig_all.tight_layout(rect=[0, 0, 1, 0.96])
