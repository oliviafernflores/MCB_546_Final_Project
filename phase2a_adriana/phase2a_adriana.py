...
def simulate_and_plot(Q, ax_full, ax_zoom):
    ...
    # df.to_csv(f"phase2a_bi_levels_Q{Q}_adriana.csv", index=False)
...
fig_full.suptitle("Phase 2a (with W-node): Full 600s", fontsize=16)
fig_zoom.suptitle("Phase 2a (with W-node): First 50s", fontsize=16)

fig_full.tight_layout(rect=[0, 0, 1, 0.97])
fig_zoom.tight_layout(rect=[0, 0, 1, 0.97])
fig_full.savefig("phase2a_full_600s_adriana.png")
fig_zoom.savefig("phase2a_zoom_0_50s_adriana.png")
plt.show()
