...
def simulate_and_plot(Q, ax_full, ax_zoom):
    ...
    # df.to_csv(f"phase2a_bi_levels_Q{Q}_sydney.csv", index=False)
...
fig_full.suptitle("Phase 2a (with X-node): Full 600s", fontsize=16)
fig_zoom.suptitle("Phase 2a (with X-node): First 50s", fontsize=16)

fig_full.tight_layout(rect=[0, 0, 1, 0.97])
fig_zoom.tight_layout(rect=[0, 0, 1, 0.97])
fig_full.savefig("phase2a_full_600s_sydney.png")
fig_zoom.savefig("phase2a_zoom_0_50s_sydney.png")
plt.show()
